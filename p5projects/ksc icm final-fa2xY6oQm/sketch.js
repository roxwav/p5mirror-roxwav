...
function setup() {
  ...
  let img = createImage("double six.jpg");
  img.loadPixels();

  drumVol = createSlider(0, 100, 70);
  drumVol.position(100, 200);

  reverb = new p5.Reverb();
  vox.disconnect();
  reverb.process(vox, 3, 2);
  reverb.amp(1.2);
...
function draw() {
  ...
  textSize(12);
  fill(50);
  text("drum volume", 100, 195);
  text("vocal reverb", 100, 235);
  text("instrument pan openness", 100, 275);

  // drum volume logic
  let v = drumVol.value() / 100;
  drum.setVolume(v);
